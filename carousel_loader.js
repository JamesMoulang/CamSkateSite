document.addEventListener('DOMContentLoaded', () => {
    // --- Configuration ---
    const CALENDAR_ID = '648a32abb0a80624c5f98e8e4bfd057578a6aed5110ba2addc6f9496fa9cabb4@group.calendar.google.com';
    ...
    const IMAGE_MAPPING = {
        ...
        'open session': ['img/advanced.jpg', 'img/open session 1.jpg', 'img/open session 2.jpg'],
        '☕ open session': 'img/advanced.jpg',
    ...
    const LINK_MAPPING = {
        ...
        'closed': [
            { href: '/sessions#beginner_sessions', text: '' }
        ],
        'girl skate night': [
        ...
        'after school club': [
            { href: '/coaching', text: 'Book coaching' }
        ],
        'after work club': [
            { href: '/coaching', text: 'Book coaching' }
        ],
        'scooters only': [
            { href: '/sessions#scooter_session', text: 'More info' }
        ],
        'after school scooter club': [
            { href: '/coaching/scooter/', text: 'More info' }
        ],
        'default': [
            { href: '/sessions', text: 'More info' }
        ]
    };
    ...
    function formatEventTime(startStr, endStr) {
        ...
        const startTime = start.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }).replace(' ', '').toLowerCase();
        const endTime = end.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }).replace(' ', '').toLowerCase();
    ...
    async function fetchAndPopulateCarousel() {
        if (API_KEY === 'YOUR_GOOGLE_API_KEY' || CALENDAR_ID === 'YOUR_CALENDAR_ID') {
            console.error("Please replace 'YOUR_GOOGLE_API_KEY' and 'YOUR_CALENDAR_ID' in carousel_loader.js");
            carousel.innerHTML = '<p style="color: red;">Please configure the Google Calendar API Key and Calendar ID.</p>';
            return;
        }
        ...
        try {
            ...
            carousel.innerHTML = '<p style="color: red;">Error loading upcoming events.</p>';
        }
    }
